...
@Component({
  ...
  template: `
   <ul *ngIf="collabsed &&data.items &&data.items.length>0"
  ...
  <li *ngFor="let item of data.items" class="sublevel-nav-item">
    <a href="" class="sublevel-nav-link" (click)="handleClick(item)" *ngIf="item.items &&item.items.length>0"
    [ngClass]="getActiveClass(item)">
    <i class="sublevel-link-icon fa fa-circle"></i>
    <span class="sublevel-link-text" *ngIf="collabsed">{{item.label}} </span>
    ...
    <div *ngIf="item.items && item.items.length>0">
      <app-sublevel-menu  [collabsed]="collabsed"]  [multiple]="multiple"] [expanded]="item.expanded" ></app-sublevel-menu>>
