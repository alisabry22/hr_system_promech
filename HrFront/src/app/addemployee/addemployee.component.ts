...
export class AddemployeeComponent implements OnInit {
  ...
  getAllEmployees() {
    this.addempservice.getAllDepartments().subscribe((response) => {
      ...
      this.final_jobs = this.jobs.map((val) => ({
        job_id: val[0],
        job_desc: val[1],
      }));
      this.sections_model = this.sections.map((val) => ({
        sect_code: val[0],
  ...
  addEmployee(event: any) {
    ...
    if (
      ...
      this.manager_email_address.length === 0 ||
      this.selectedSection.length === 0 ||
      this.card_id.length == 0 ||
      this.selectedCompany.length == 0 ||
      this.employeeType.length == 0 ||
      this.ordinaryVacation.length == 0 ||
      this.casualVacation.length == 0 ||
      this.selectedDebt.length == 0 ||
      this.employeeType.length == 0 ||
      this.hiredate.toString().length == 0
    ) {
      this.state = 'error';
      ...
      if (dep && jobid) {
        this.addempservice
          .addnewEmployee(
            ...
            this.employeeType,
            this.hiredate,
            this.ordinaryVacation,
            this.casualVacation,
            this.selectedCompany,
            this.card_id,
            this.selectedSection,
            this.email_address,
            this.manager_email_address
          )
          .subscribe({
            next: (event: any) => {
