...
if(!fileType || !file ){
        ...
        return res.send({state:"error",message:"Some Values are not Specified"}); 
}
...
try {
    ...
    return res.send({state:"success",message:" Successfully uploaded file"});
} catch (error) {
    console.log(error);
...
const penta3d=async(req,res)=>{
    ...
    if(file.originalname!="penta3d.xlsx"){
        return res.send({state:"error",message:"Wrong File Type for Penta3d"}); 
    }   
...
const promech12=async(req,res)=>{
    ...
    if(file.originalname!="promech12.xlsx"){
        return res.send({state:"error",message:"Wrong File Type for Promech12"}); 
    }
